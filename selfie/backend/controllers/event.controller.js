...
export const deleteEvent = async (req, res) => {
  ...
  try {
    ...
    if (!deleted) return res.status(404).json({ error: "Event not found or not yours" });
...
export const excludeOccurrence = async (req, res) => {
  ...
  if (!dateToExclude) return res.status(400).json({ error: "Missing exclusion date in request body" });

  try {
    const event = await Event.findById(id);
    if (!event) return res.status(404).json({ error: "Event series not found" });
