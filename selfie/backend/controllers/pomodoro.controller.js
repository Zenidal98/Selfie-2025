...
export const createPomodoroSession = async (req, res) => {
  try {
    const session = new Pomodoro(req.body);
    const saved = await session.save();
    res.status(201).json({ success: true, data: saved });
  } catch (err) {
    res.status(500).json({ success: false, message: "Errore nel salvataggio", error: err.message });
  }
};
...
export const getLastPomodoroByUser = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ success: false, message: "Errore nel recupero", error: err.message });
  }
};
