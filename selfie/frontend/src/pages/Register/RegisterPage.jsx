...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const passwordinserita = formData.password;
    const confermainserita = formData.conferma;
    ...
    if (passwordinserita !== confermainserita) {
      alert("Controlla di aver confermato bene la password.");
      return;
    }
    ...
    try { const response = await api.post("/register",formData);
      ...
      if (error.response) {
        const msg = error.response.data?.message || "Errore sconosciuto dal server.";  // tutti i vari errori scritti in register controller
        alert(msg); 
      } else if (error.request) {
        alert("Nessuna risposta dal server. Verifica che sia avviato.");
      } 
    }
  };
