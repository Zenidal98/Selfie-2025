...
const PomodoroTimer = ({ studyDuration, breakDuration, cycles }) => {
  ...
  const toggleTimer = () => {
    if (isComplete) return; // 🚫 block if session ended
    if (isRunning) {
      clearInterval(intervalRef.current);
    } else {
  ...
  return (
    <div className="pomodoro-timer mb-4 p-3 shadow rounded">
      ...
      <div className="d-flex flex-wrap justify-content-center gap-2 mt-3">
        ...
        <button className="btn btn-secondary" onClick={resetTimer} disabled={isComplete}>Reset</button>
        <button className="btn btn-warning" onClick={nextTime} disabled={isComplete}>Prossimo tempo</button>
        <button className="btn btn-info" onClick={restartCycle} disabled={isComplete}>Ricomincia ciclo</button>
        <button className="btn btn-danger" onClick={finishCycle} disabled={isComplete}>Termina ciclo</button>
      </div>
    </div>
  );
};
